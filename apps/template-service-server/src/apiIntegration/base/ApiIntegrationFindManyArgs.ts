/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ApiIntegrationWhereInput } from "./ApiIntegrationWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ApiIntegrationOrderByInput } from "./ApiIntegrationOrderByInput";

@ArgsType()
class ApiIntegrationFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ApiIntegrationWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ApiIntegrationWhereInput, { nullable: true })
  @Type(() => ApiIntegrationWhereInput)
  where?: ApiIntegrationWhereInput;

  @ApiProperty({
    required: false,
    type: [ApiIntegrationOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ApiIntegrationOrderByInput], { nullable: true })
  @Type(() => ApiIntegrationOrderByInput)
  orderBy?: Array<ApiIntegrationOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ApiIntegrationFindManyArgs as ApiIntegrationFindManyArgs };
